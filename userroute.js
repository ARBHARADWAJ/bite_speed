...
const Contact = require("./fucntions");

const identify = async (req, res) => {
    console.log("displaying the req");
  ...
  try {
    ...
    if (primaryContact == "Primary Contact not found") {
      primaryContact = await Contact.createContact({
        email,
        phoneNumber,
        linkedId: null,
      ...
      // Check if the this request has email and phoneNumber from two different existing contacts
      const updateSecondaryContact = primaryContact[1];
      primaryContact = primaryContact[0];

      //Update the contact as Secondary
      await Contact.updateContactAsSecondary(
        primaryContact.id,
        updateSecondaryContact.id
      );
    } else {
      // Create request as Secondary Contact
      _primaryContact = primaryContact[0];
      secondaryContacts ==
        (await Contact.createContact({
          email,
          phoneNumber,
          linkedId: _primaryContact.id,
    ...
    const getSecondaryContacts = await Contact.getSecondaryContacts(
      primaryContact.id
    );
    ...
    if (Array.isArray(getSecondaryContacts)) {
      ...
      secondaryPhoneNumbers = getSecondaryContacts.map(
